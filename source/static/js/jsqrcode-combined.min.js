GridSampler = {
    checkAndNudgePoints: function (d, a) {
        for (var c = qrcode.width, b = qrcode.height, e = !0, g = 0; g < a.Length && e; g += 2) {
            var f = Math.floor(a[g]), h = Math.floor(a[g + 1]);
            if (-1 > f || f > c || -1 > h || h > b)throw"Error.checkAndNudgePoints ";
            e = !1;
            -1 == f ? (a[g] = 0, e = !0) : f == c && (a[g] = c - 1, e = !0);
            -1 == h ? (a[g + 1] = 0, e = !0) : h == b && (a[g + 1] = b - 1, e = !0)
        }
        e = !0;
        ...
        for (var b = new BitMatrix(a), e = Array(a << 1), g = 0; g < a; g++) {
            ...
            try {
                for (j = 0; j < f; j += 2) {
                    var k = 4 * Math.floor(e[j]) + 4 * Math.floor(e[j + 1]) * qrcode.width, n = d[Math.floor(e[j]) + qrcode.width * Math.floor(e[j + 1])];
...
function Version(d, a, c, b, e, g) {
    ...
    this.buildFunctionPattern = function () {
        var b = this.DimensionForVersion, e = new BitMatrix(b);
        e.setRegion(0, 0, 9, 9);
        e.setRegion(b - 8, 0, 8, 9);
        e.setRegion(0, b - 8, 9, 8);
        for (var c = this.alignmentPatternCenters.length, a = 0; a < c; a++)for (var g = this.alignmentPatternCenters[a] - 2, d = 0; d < c; d++)0 == a && (0 == d || d == c - 1) || a == c - 1 && 0 == d || e.setRegion(this.alignmentPatternCenters[d] - 2, g, 5, 5);
        e.setRegion(6, 9, 1, b - 17);
        e.setRegion(9, 6, b - 17, 1);
        6 < this.versionNumber && (e.setRegion(b - 11, 0, 3, 6), e.setRegion(0,
            b - 11, 6, 3));
        return e
    };
...
function PerspectiveTransform(d, a, c, b, e, g, f, h, j) {
    ...
    this.transformPoints2 = function (b, e) {
        for (var c = b.length, a = 0; a < c; a++) {
            var g = b[a], d = e[a], f = this.a13 * g + this.a23 * d + this.a33;
...
function Detector(d) {
    ...
    this.sizeOfBlackWhiteBlackRunBothWays =
        function (a, c, b, e) {
            var g = this.sizeOfBlackWhiteBlackRun(a, c, b, e), d = 1;
            b = a - (b - a);
    ...
    this.processFinderPatternInfo = function (a) {
        var c = a.TopLeft, b = a.TopRight;
        a = a.BottomLeft;
        var e = this.calculateModuleSize(c, b, a);
        if (1 > e)throw"Error";
        var d = this.computeDimension(c, b, a, e), f = Version.getProvisionalVersionForDimension(d), h = f.DimensionForVersion - 7, j = null;
...
FormatInformation.doDecodeFormatInformation = function (d) {
    for (var a = 4294967295, c = 0, b = 0; b < FORMAT_INFO_DECODE_LOOKUP.length; b++) {
        var e = FORMAT_INFO_DECODE_LOOKUP[b], g = e[0];
...
function BitMatrix(d, a) {
    ...
    this.setRegion = function (b, e, c, a) {
        ...
        for (; e < a; e++)for (var d = e * this.rowSize, j = b; j < c; j++)this.bits[d + (j >> 5)] |= 1 << (j & 31)
    }
};
...
DataBlock.getDataBlocks = function (d, a, c) {
    ...
    for (var g = 0, f = 0; f < e.length; f++) {
        var h = e[f];
        for (a = 0; a < h.Count; a++) {
            var j = h.DataCodewords, k = b.ECCodewordsPerBlock + j;
    ...
    for (a = h = 0; a < b; a++)for (f = 0; f < g; f++)c[f].codewords[a] =
        d[h++];
    for (f = e; f < g; f++)c[f].codewords[b] = d[h++];
    j = c[0].codewords.length;
    for (a = b; a < j; a++)for (f = 0; f < g; f++)c[f].codewords[f < e ? a : a + 1] = d[h++];
    return c
};
function BitMatrixParser(d) {
    ...
    this.readVersion = function () {
        ...
        for (var b = 0, e = c - 11, a = 5; 0 <= a; a--)for (var d = c - 9; d >= e; d--)b = this.copyBit(d, a, b);
        ...
        b = 0;
        for (d = 5; 0 <= d; d--)for (a = c - 9; a >= e; a--)b = this.copyBit(d, a, b);
        this.parsedVersion = Version.decodeVersionInformation(b);
        if (null != this.parsedVersion && this.parsedVersion.DimensionForVersion == c)return this.parsedVersion;
        throw"Error readVersion";
    };
    this.readCodewords = function () {
        var c = this.readFormatInformation(), b = this.readVersion(), e = DataMask.forReference(c.DataMask), c = this.bitMatrix.Dimension;
        ...
        for (var e = b.buildFunctionPattern(), a = !0, d = Array(b.TotalCodewords), h = 0, j = 0, k = 0, n = c - 1; 0 < n; n -= 2) {
            6 == n && n--;
            for (var l = 0; l < c; l++)for (var s = a ? c - 1 - l : l, r = 0; 2 > r; r++)e.get_Renamed(n - r, s) || (k++, j <<= 1, this.bitMatrix.get_Renamed(n - r, s) && (j |= 1), 8 == k && (d[h++] = j, j = k = 0));
            a ^= 1
        }
...
function DataMask000() {
    this.unmaskBitMatrix = function (d, a) {
        for (var c = 0; c < a; c++)for (var b = 0; b < a; b++)this.isMasked(c, b) && d.flip(b, c)
    };
    ...
    this.runEuclideanAlgorithm = function (a, c, b) {
        ...
        for (var e = this.field.One, d = this.field.Zero, f = this.field.Zero, h = this.field.One; c.Degree >= Math.floor(b / 2);) {
            ...
            for (d = this.field.inverse(d); c.Degree >= a.Degree && !c.Zero;) {
                var j = c.Degree - a.Degree, l = this.field.multiply(c.getCoefficient(c.Degree), d), h = h.addOrSubtract(this.field.buildMonomial(j,
                    l));
...
function GF256Poly(d, a) {
    ...
    if (1 < c && 0 == a[0]) {
        ...
        if (b == c)this.coefficients = d.Zero.coefficients; else {
    ...
    this.multiply1 = function (b) {
        ...
        var a = this.coefficients, c =
            a.length;
        b = b.coefficients;
        for (var d = b.length, j = Array(c + d - 1), k = 0; k < c; k++)for (var n = a[k], l = 0; l < d; l++)j[k + l] = GF256.addOrSubtract(j[k + l], this.field.multiply(n, b[l]));
    ...
    this.divide = function (b) {
        ...
        for (var a = this.field.Zero, c = this, d = b.getCoefficient(b.Degree), d = this.field.inverse(d); c.Degree >= b.Degree && !c.Zero;)var j = c.Degree - b.Degree, k = this.field.multiply(c.getCoefficient(c.Degree),
            d), n = b.multiplyByMonomial(j, k), j = this.field.buildMonomial(j, k), a = a.addOrSubtract(j), c = c.addOrSubtract(n);
        return [a, c]
    }
};
function GF256(d) {
...
Decoder.decode = function (d) {
    ...
    for (var b = Array(b), g = 0, f = 0; f < a.length; f++) {
        var e = a[f], h = e.Codewords, j = e.NumDataCodewords;
...
qrcode = {
    ...
    decode: function (d) {
        if (0 == arguments.length) {
            var a = document.getElementById("qr-canvas"), c = a.getContext("2d");
    ...
    process: function (d) {
        var a = (new Date).getTime(), c = qrcode.grayScaleToBitmap(qrcode.grayscale());
        if (qrcode.debug) {
            for (var b = 0; b < qrcode.height; b++)for (var e = 0; e < qrcode.width; e++) {
        ...
        d = "";
        for (b = 0; b < c.length; b++)for (e = 0; e < c[b].length; e++)d += String.fromCharCode(c[b][e]);
    ...
    binarize: function (d) {
        for (var a = Array(qrcode.width * qrcode.height), c = 0; c < qrcode.height; c++)for (var b = 0; b < qrcode.width; b++) {
    ...
    getMiddleBrightnessPerArea: function (d) {
        ...
        for (e = 0; 4 > e; e++)for (g = 0; 4 > g; g++) {
            b[g][e][0] = 255;
            for (var f = 0; f <
            c; f++)for (var h = 0; h < a; h++) {
        ...
        for (e = 0; 4 > e; e++)for (g = 0; 4 > g; g++)d[g][e] = Math.floor((b[g][e][0] + b[g][e][1]) / 2);
        return d
    },
    grayScaleToBitmap: function (d) {
        for (var a = qrcode.getMiddleBrightnessPerArea(d), c = a.length, b = Math.floor(qrcode.width / c), e = Math.floor(qrcode.height / c), g = Array(qrcode.height * qrcode.width), f = 0; f < c; f++)for (var h = 0; h < c; h++)for (var j = 0; j < e; j++)for (var k = 0; k < b; k++)g[b *
        h + k + (e * f + j) * qrcode.width] = d[b * h + k + (e * f + j) * qrcode.width] < a[h][f] ? !0 : !1;
        return g
    },
    grayscale: function () {
        for (var d = Array(qrcode.width * qrcode.height), a = 0; a < qrcode.height; a++)for (var c = 0; c < qrcode.width; c++) {
...
qrcode.orderBestPatterns = function (d) {
    ...
    var c = a(d[0], d[1]), b = a(d[1], d[2]), e = a(d[0], d[2]);
...
function FinderPatternFinder() {
    ...
    this.findRowSkip = function () {
        ...
        for (var a = null, c = 0; c < d; c++) {
            ...
            if (b.Count >= CENTER_QUORUM)if (null == a)a = b; else return this.hasSkipped = !0, Math.floor((Math.abs(a.X - b.X) - Math.abs(a.Y - b.Y)) / 2)
        }
        return 0
    };
    ...
    this.findFinderPattern = function (d) {
        this.image = d;
        var a = qrcode.height, c = qrcode.width, b = Math.floor(3 * a / (4 * MAX_MODULES));
        ...
        for (var e = !1, g = Array(5), f = b - 1; f < a && !e; f += b) {
            ...
            g[2] = 0;
            g[3] = 0;
            for (var h = g[4] = 0, j = 0; j < c; j++)if (d[j + f * qrcode.width])1 == (h & 1) && h++, g[h]++; else if (0 == (h & 1))if (4 == h)if (this.foundPatternCross(g)) {
                if (h =
                        this.handlePossibleCenter(g, f, j))b = 2, this.hasSkipped ? e = this.haveMultiplyConfirmedCenters() : (h = this.findRowSkip(), h > g[2] && (f += h - g[2] - b, j = c - 1)); else {
                ...
                g[3] = 0;
                g[4] = 0
            } else g[0] = g[2], g[1] = g[3], g[2] = g[4], g[3] = 1, g[4] = 0, h = 3; else g[++h]++; else g[h]++;
            if (this.foundPatternCross(g) && (h = this.handlePossibleCenter(g, f, c)))b = g[0], this.hasSkipped && (e = haveMultiplyConfirmedCenters())
        }
...
function AlignmentPatternFinder(d, a, c, b, e, g, f) {
    ...
    this.foundPatternCross = function (b) {
        for (var a = this.moduleSize, c = a / 2, d = 0; 3 > d; d++)if (Math.abs(a - b[d]) >= c)return !1;
        return !0
    };
    ...
    this.handlePossibleCenter = function (b, a, c) {
        ...
        if (!isNaN(a)) {
            b = (b[0] + b[1] + b[2]) / 3;
            for (var d = this.possibleCenters.length, e = 0; e < d; e++)if (this.possibleCenters[e].aboutEquals(b, a, c))return new AlignmentPattern(c, a, b);
...
function QRCodeDataBlockReader(d, a, c) {
    ...
    this.getRomanAndFigureString =
        function (b) {
            var a = 0, c = "", d = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:".split("");
            do if (1 < b) {
                var a = this.getNextBits(11), h = a % 45, c = c + d[Math.floor(a / 45)], c = c + d[h];
                b -= 2
            } else 1 == b && (a = this.getNextBits(6), c += d[a], b -= 1); while (0 < b);
            return c
        };
    ...
    this.getKanjiString = function (b) {
        var a = 0, c = "";
        do {
            var a = getNextBits(13), a = (a / 192 << 8) + a % 192, d = 0, d = 40956 >= a + 33088 ? a + 33088 : a + 49472, c = c + String.fromCharCode(d);
            b--
        } while (0 < b);
        return c
    };
    this.__defineGetter__("DataByte", function () {
        ...
        do {
            var a = this.NextMode();
            if (0 == a)if (0 < b.length)break; else throw"Empty data block";
